...
class EventDetailScreen extends StatelessWidget {
  ...
  Future<void> _openLink(BuildContext context, String rawUrl) async {
    ...
    if (url.isEmpty || !url.startsWith("http")) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Invalid event link")),
      );
      return;
    }
  ...
  Future<void> _saveEvent(BuildContext context) async {
    ...
    if (existing.docs.isNotEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Event already saved")),
      );
      return;
    }

    await FirebaseFirestore.instance.collection('saved_events').add({
      'eventId': eventId,
      'userId': uid,
                        ...
                        children: [
                          ...
                          Expanded(
                            child: ElevatedButton(
                              onPressed: () => _openLink(context, d['link'] ?? ""),
