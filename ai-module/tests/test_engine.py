...
def run_test(name, profile_text, jd_text, required_exp, expected_min_score=None):
    ...
    print(f"\n[Score] Career Score     : {cs['career_score']} / 100  ->  {to_ascii(cs.get('tier', ''))}")
    ...
    print(f"\n[Skills] Matched Skills  : {to_ascii(sp['matched_skills'])}")
    ...
    for r in sp["suggested_roles"]:
        print(f"   -> {to_ascii(r['role'])}  ({r['match_percent']}% match)")
    ...
    for res in sp["learning_resources"]:
        print(f"   * {to_ascii(res['skill'])}: {to_ascii(res['resource'])}")
